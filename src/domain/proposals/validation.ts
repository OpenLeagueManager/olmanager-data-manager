...
export function createValidator(
  ...
  social: { accounts: SocialAccountData[]; templates: SocialTemplateData[] },
) {
  function validateProposal(input: unknown): ValidationResult<ProposalPayload> {
    if (!isRecord(input)) {
  ...
  function validateTransferPlayer(input: RawRecord): ValidationResult<TransferPlayerPayload> {
    ...
    const comp = game.manifests.find((m) => m.id === competitionId);
  ...
  function validateEditCompetition(input: RawRecord): ValidationResult<EditCompetitionPayload> {
    ...
    if (competitionId && !game.manifests.some((m) => m.id === competitionId)) {
  ...
  function validateEditSocialTemplate(input: RawRecord): ValidationResult<EditSocialTemplatePayload> {
    ...
    const template = social.templates.find((candidate) => candidate.id === templateId);
